...
export default function AddDealModal({ open, onOpenChange, categories }: AddDealModalProps) {
  ...
  const addDealMutation = useMutation({
    ...
    onSuccess: (data) => {
      // Invalidate and refetch deals to update the UI
      queryClient.invalidateQueries({ queryKey: ["deals"] })

      // Add the new deal to the cache for immediate display
      /* eslint-disable @typescript-eslint/no-explicit-any */
      queryClient.setQueryData(["deals"], (oldData: any) => {
        if (oldData && Array.isArray(oldData)) {
          return [data, ...oldData]
        } else if (oldData && oldData.deals && Array.isArray(oldData.deals)) {
          return {
            ...oldData,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    formData.append("offers", JSON.stringify(offers.filter((offer) => offer.trim() !== "")))
      ...
      <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold">
            Deals
            <div className="text-base font-normal text-[#595959] mt-1">Dashboard &gt; Deals &gt; Add Deal</div>
          </DialogTitle>
        </DialogHeader>
            ...
            <div>
              ...
              {offers.map((offer, index) => (
                <div key={index} className="flex items-center gap-2 mt-2">
                  ...
                  />
                  {offers.length > 1 && (
                    <Button type="button" variant="ghost" size="icon" onClick={() => removeOffer(index)}>
                      <X className="h-4 w-4" />
                    </Button>
                  )}
                </div>
              ))}
              <Button type="button" variant="outline" size="sm" onClick={addOffer} className="mt-2">
                Add Offer
              </Button>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="border-2 border-dashed rounded-lg p-4 mt-1 text-center">
                ...
                <div className="flex flex-col items-center justify-center min-h-[150px]">
                  ...
                  <p className="text-sm text-gray-500 mb-2">Drag and drop image here, or click add image</p>
                ...
                <div className="grid grid-cols-5 gap-2 mt-4">
                  {[...Array(5)].map((_, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`relative border ${hasImage ? "border-gray-300" : "border-dashed border-gray-300"
                          } rounded p-1 h-20`}
                      >
                        {hasImage ? (
                          <>
