...
//             <StripeCheckout userId={session?.user?.id ?? ""} bookingId={bookingId ?? ""} amount={pric
//           {/* </Elements> */}
//         </DialogContent>
//       </Dialog>
//     </>
...
export function DealsCard({
  ...
  const createPaymentIntent = useMutation({
    mutationFn: async () => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/stripe/create-payment`, {
        ...
        body: JSON.stringify({ userId: session?.user?.id,bookingId,price }),
      });
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (selectedPaymentMethod === "paypal") {
          ...
          createPaymentIntent.mutate(); // Trigger payment intent creation
        }
      } else {
        const error = await response.json();
  ...
  // Open Stripe modal when clientSecret is set
  useEffect(() => {
    if (clientSecret && selectedPaymentMethod === "stripe") {
      setIsStripeModalOpen(true);
    }
  }, [clientSecret, selectedPaymentMethod]);
