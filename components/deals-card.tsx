...
import { ChevronRight, MapPin, Calendar, X } from "lucide-react";
...
// import type { ScheduleDate, Location } from "@/types";
...
interface DealsCardProps {
  ...
  bookingId?: string;
  onRemoveNotification?: (bookingId: string) => void;
}
...
export function DealsCard({
  ...
  useEffect(() => {
    if (isStripeModalOpen && newBookingId && !clientSecret) {
      createPaymentIntent();
    }
  }, [isStripeModalOpen, newBookingId]);
  ...
  const handleBooking = async (notifyMe: boolean) => {
    if (!session?.user?.id) {
      toast.success("Bitte melde dich an, um diese Funktion zu nutzen");
      return;
    }
  ...
  const bookingPayment = async (notifyMe = false) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewBookingId(data.booking._id);
        setIsBookingSummaryOpen(false);
  ...
  const renderActionButton = () => {
    if (showRemoveNotification && bookingId && onRemoveNotification) {
      return (
        <Button
          className="w-full bg-red-600 text-white font-semibold mt-2 hover:bg-red-700 flex items-center justify-center gap-2"
          onClick={() => onRemoveNotification(bookingId)}
        >
          <X className="w-4 h-4" />
