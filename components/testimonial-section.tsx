...
export function TestimonialSection() {
  ...
  const handlePrev = () => {
    ...
    setCurrentIndex((prev) => (prev - 1 + data.feedbacks.length) % data.feedbacks.length)
  }
  ...
  if (isError || !data?.success) {
    return (
      <section className="container lg:mt-24 py-[50px]">
        <div className="text-white text-center">Failed to load feedback. Please try again later.</div>
      </section>
    )
  }
  ...
  if (!data.feedbacks || data.feedbacks.length === 0) {
    return (
      <section className="container lg:mt-24 py-[50px]">
        <div className="text-white text-center">No feedback available at the moment.</div>
      </section>
    )
  }
    ...
    <section className="container lg:mt-24 py-[50px]">
      <div className="grid gap-8 grid-cols-6">
        <div className="col-span-6 md:col-span-2 lg:col-span-2">
          ...
          <p className="text-white text-sm mt-2">{data.feedbacks.length} testimonials received</p>
        </div>
        ...
        <div className="col-span-6 md:col-span-4 lg:col-span-4">
          <div className="relative mb-5 pl-6">
            ...
            <p className="text-white text-lg italic text-muted-foreground">{feedback.message}</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="relative h-12 w-12 overflow-hidden rounded-full bg-gray-700 flex items-center justify-center">
              ...
              <span className="text-white text-lg font-bold">{feedback.name.charAt(0)}</span>
            </div>
            <div>
              <h4 className="text-white text-sm lg:text-lg font-medium">{feedback.name}</h4>
              <p className="text-white text-sm text-muted-foreground">{formattedDate}</p>
            </div>
